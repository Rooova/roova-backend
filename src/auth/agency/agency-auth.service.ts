...
const RESET_URL_BASE =
  process.env.AGENCY_RESET_PASSWORD_URL ?? 'http://agency.localhost:3000/reset-password';

function toPublicAgency(agency: Agency) {
  const { passwordHash, passwordResetTokenHash, passwordResetExpiresAt, ...rest } = agency;
  return rest;
}
...
export class AgencyAuthService {
  ...
  async resetPassword(dto: ResetPasswordDto) {
    ...
    if (!agency || !agency.passwordResetExpiresAt || agency.passwordResetExpiresAt < new Date()) {
